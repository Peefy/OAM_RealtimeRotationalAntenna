...
Sigma1 = ones(1,nScatter1); % RCS 
% 随机生成，在一个区间内均匀分布 x ~ [-.1,.1] y~[10.1 10.3], z~ [-.1 .1];
PosScatter1(1,:) =0.1*(rand(1,nScatter1)-0.5);
PosScatter1(2,:) = 0.1*rand(1,nScatter1)+10.2;
PosScatter1(3,:) = 0.1*(rand(1,nScatter1)-0.5);
% 物体2 
nScatter2 = 20;
PosScatter2 = zeros(3,nScatter2);
Sigma2 = ones(1,nScatter2); % RCS 
% 随机生成，在一个区间内均匀分布 x ~ [-.1,.1] y~[10.8 11], z~ [-.1 .1];
PosScatter2(1,:) = 0.1*(rand(1,nScatter2)-0.5);
PosScatter2(2,:) = 0.1*rand(1,nScatter2)+10.8;
PosScatter2(3,:) = 0.1*(rand(1,nScatter2)-0.5);
% 总体
...
figure 
plot(T*c/2+ts*c/2,SignalMatched)