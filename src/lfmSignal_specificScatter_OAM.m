...
radius1 = .2;
smallAngle = 10*pi/180;
PosScatter1 = [cos(smallAngle),-cos(smallAngle);
               sin(smallAngle),-sin(smallAngle);
               0              ,0               ;];
PosScatter1 = PosScatter1 + repmat([0;10.2;0],[1,nScatter1]);
% 物体2 ;
nScatter2 = 2;
...
radius2 = .2;
smallAngle = 10*pi/180;
PosScatter2 = [cos(smallAngle),-cos(smallAngle);
               sin(smallAngle),-sin(smallAngle);
               0              ,0               ;];
PosScatter2 = PosScatter2 + repmat([0;10.8;0],[1,nScatter2]);
% 总体
...
radius = 0.15; % m 发射天线半径
angleStep = 2*pi/180;% rad 步进角度
Angle = 0:angleStep:2*pi-angleStep;
nA = length(Angle);
SignalAll = zeros(nA,nT);

for iA = 1:nA
    
PosTrans = [radius*cos(Angle(iA));radius*sin(Angle(iA));0]; %
PosRecev = [0;0;0]; % 
...
SignalAll(iA,:) = SignalMatched;
end 
save SignalAll180 SignalAll;
[AGrid,TGrid] = meshgrid(Angle,T);
figure 
mesh(AGrid,TGrid,SignalAll.');

SignalOAM = abs((fft(SignalAll)));
Max1 = max(SignalOAM.').';
SignalOAMNorm = SignalOAM./repmat(Max1,[1,nT]);
figure
mesh(AGrid,TGrid,SignalOAM.');
figure 
mesh(AGrid,TGrid,SignalOAMNorm.');
for iA = 1:nA
figure 
plot(T*c/2,SignalOAMNorm(iA,:))
xlabel('距离（m）')
ylabel('幅值')
pause(.2)
end 